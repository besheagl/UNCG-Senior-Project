...
<html>
    <head>
        <!--This page is for adding a show to the Watched list-->
        <title>Armchair Add Show to Watched</title>
	...
	<main>
		<span class="listtitle"><h3>Add Show to Watched</h3></span>
			<div id = 'addwatched'>
			<a href = "https://armchair.000webhostapp.com/profileshows.php/?dispusername=<?php echo $username; ?>" class = "backbtn">Back to profile</a>
				<form method="post" action="https://armchair.000webhostapp.com/addShowToWatched.php">
					<fieldset><legend>Select Show to Add to Watched</legend>
						<select name = 'showToAdd' style = 'width: 90%; height: 50px;'>
							...
							<input type = "hidden" name = "goback" value = "<?php echo 'https://armchair.000webhostapp.com/addshowwatched.php' ?>">
						</select>
						<br><br>
				...
				<?php
				    if (isset($_SESSION['addwatched'])){
					    $addread = $_SESSION['addwatched'];
					if ($addread){
				?>
				<span class = 'feedback'>
					<?php echo "Added ".getShowInfo($con, $addread)[1]." to watched!"; ?>
				</span>
				<?php
					} unset($_SESSION['addwatched']); 
				}
				?>
			</div>
	</main>
