...
<html>
    <head>
        <!--This page is for adding a show to the Favorite Shows List-->
		...
		<main>
			<span class="listtitle"><h3>Add Show</h3></span>
			    <div id = 'addfavorites'>
				    ...
				    <?php
				        if (isset($_SESSION['addfavshow'])){
					        $addread = $_SESSION['addfavshow'];
					    if ($addread){
				    ?>
					<span class = 'feedback'>
					    <?php echo "Added ".getShowInfo($con, $addread)[1]." to favorites!"; ?>
					</span>
				    <?php
					    } unset($_SESSION['addfavshow']); 
				    }
				    ?>
			    </div>
		</main>
